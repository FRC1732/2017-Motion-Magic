...
public class Drivetrain extends Subsystem {
	...
	public static final double DEFAULT_VELOCITY = 450; // RPM
	...
	private void configureATalonEncoder(CANTalon talon, boolean reverseSensor, int defaultProfile) {
		...
		/*
		 * talon.setNominalClosedLoopVoltage does the same thing for closed loop as what
		 * voltage compensation mode does for teleop
		 */
	...
	private void setMotionMagicPID() {
		...
		leftMaster.setPID(p * leftPScale, i * leftIScale, d * leftDScale * leftPScale, fLeft * leftFScale, IZONE,
				CLOSED_LOOP_RAMP_RATE, DEFAULT_PROFILE);
		rightMaster.setPID(p * rightPScale, i * rightIScale, d * rightDScale * rightPScale, fRight * rightFScale, IZONE,
				CLOSED_LOOP_RAMP_RATE, DEFAULT_PROFILE);
	}
	...
	public void updateMotionMagicPID() {
		...
		System.out.printf("%.2f %.2f", fLeft * lf, fRight * rf);

		leftMaster.setPID(p * lp, i * li, d * ld * lp, fLeft * lf, IZONE, CLOSED_LOOP_RAMP_RATE, DEFAULT_PROFILE);
		rightMaster.setPID(p * rp, i * ri, d * rd * rp, fRight * rf, IZONE, CLOSED_LOOP_RAMP_RATE, DEFAULT_PROFILE);
	}
